...
class RsRpc(object):
  ...
  def response(self, req_id, timeout):
    if __debug__:
        print(f"RsRpc::response: req_id: {reg_id}, timeout: {timeout}")
    # special case - no timeout, non-blocking.
    if (not timeout):
      if __debug__:
        print(f"RsRpc::response: no timeout => get & return")
    ...
    while(now < expiry_time):
      if __debug__:
        print (f"RsRpc::response: try fetch cycle...")
    ...
    if __debug__:
        print (f"RsRpc::response: timeout: No Response")
    return None
  ...
  def response_stored(self, req_id):
    if req_id in self.responses:
      ...
      if __debug__:
        print (f"RsRpc::response_stored({reg_id}) Found!")
    return ans
    if __debug__:
        print (f"RsRpc::response_stored({reg_id}) No received!")
    return None
  ...
  def fetch_responses(self):
    if __debug__:
        print(f"RsRpc::fetch_responses()")
    if self.comms.recv_ready():
      if __debug__:
        print (f"RsRpc::fetch_responses() Data Ready")
      ans = self.msgpacker.read_msg()
      if not ans:
        if __debug__:
            print (f"RsRpc::fetch_responses() Bad Answer")
        return None
      ...
      if __debug__:
        print(f"RsRpc::fetch_responses() Storing Answer ({msg_type}, {req_id}, len({len(msg_body)})")
